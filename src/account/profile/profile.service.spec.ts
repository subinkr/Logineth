...
describe('ProfileService', () => {
  ...
  // CWTEST: - use, return, error
  describe('Connect Wallet', () => {
    const reqConnectWallet: ReqConnectWallet = {
      wallet: user.wallet,
    };
    const resConnectWallet: ResConnectWallet = { message: '연결되었습니다.' };
    let result = {};

    it('Use | getUserByID', async () => {
      service.getUserByID = jest.fn().mockReturnValue(user);
      result = await service.connectWallet(user.id, reqConnectWallet, user.id);
      expect(service.getUserByID).toHaveBeenCalled();
    });

    it('Return | ResConnectWallet', async () => {
      const keys = Object.keys(result);
      const required = Object.keys(resConnectWallet);
      expect(keys).toEqual(expect.arrayContaining(required));
    });

    it('Error | Cannot edit other user.', async () => {
      const result = service.connectWallet(
        user.id,
        reqConnectWallet,
        otherUser.id,
      );
